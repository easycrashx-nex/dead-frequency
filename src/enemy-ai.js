...
const eye = value => ({ x: value.x, y: 1.55, z: value.z });
const soundPoint = value => ({ x: Math.round(value.x / 3) * 3, z: Math.round(value.z / 3) * 3 });

export function createEnemyAI({ isWalkable, findPath, hasLineOfSight, walkSegmentClear, colliders = [], random = Math.random, shoot = () => {} }) {
  const coverPoints = [];
  let routeTokens = 2, metrics = { plans: 0, senses: 0, routes: 0 };
  const clear = (a, b) => walkSegmentClear(a, b, .38);
  for (const box of colliders) {
    if (box.y - box.h / 2 > .1 || box.y + box.h / 2 < 1.9) continue;
    for (const axis of ['x', 'z']) {
      const tangent = axis === 'x' ? 'z' : 'x', width = (axis === 'x' ? box.w : box.d) / 2, length = (axis === 'x' ? box.d : box.w) / 2;
      for (const side of [-1, 1]) for (const end of [-1, 1]) {
        const hide = { [axis]: box[axis] + side * (width + .75), [tangent]: box[tangent] + end * (length - Math.min(.65, length * .6)) };
        const peek = { [axis]: hide[axis], [tangent]: box[tangent] + end * (length + 1.05) };
  ...
  function initialize(e) {
    ...
    return e.ai = { role: e.flank ? 'flanker' : 'anchor', task: initialContact ? 'investigate' : 'patrol',
  ...
  function hearOne(e, location, kind, playerId, radius) {
    ...
    const direct = hasLineOfSight(eye(e), { ...location, y: location.y ?? 1.4 });
  ...
  function hear(enemies, location, { kind = 'shot', playerId = null, radius = 36 } = {}) {
  ...
  function freeGoal(value) {
    if (isWalkable(value.x, value.z, .4)) return point(value);
    for (const radius of [.65, 1.3, 2.6]) for (let i = 0; i < 8; i++) {
      const candidate = { x: value.x + Math.cos(i * Math.PI / 4) * radius, z: value.z + Math.sin(i * Math.PI / 4) * radius };
      if (isWalkable(candidate.x, candidate.z, .4)) return candidate;
    }
    return null;
  }
  function setGoal(e, goal) {
    ...
    if (!ai.goal || distance(ai.goal, goal) > .4) { ai.goal = point(goal); e.path = []; }
  }
  ...
  function seekCover(e, contact, enemies) {
    ...
    for (const cover of candidates.slice(0, 10)) {
      ...
      if (hasLineOfSight(eye(cover.hide), { ...contact, y: 1.3 }) || !hasLineOfSight(eye(cover.peek), { ...contact, y: 1.3 })) continue;
  ...
  function share(e, enemies) {
    ...
    ai.shareTimer = 2.2;
    const nearby = enemies.filter(other => other !== e && !other.dead && distance(other, e) < 24)
      .sort((a, b) => distance(a, e) - distance(b, e));
    let informed = 0;
    for (const other of nearby) {
      const brain = initialize(other);
      if (brain.visible || !hasLineOfSight(eye(e), eye(other))) continue;
  ...
  function sense(e, targets, enemies, difficulty) {
    ...
    for (const target of targets) {
      ...
      const range = (difficulty === 'hard' ? 36 : 30) * (p.crouching && !p.moving ? .65 : 1);
  ...
  function plan(e, enemies) {
    ...
    if (known && (ai.visible || (ai.cover && ai.contactAge < ai.memoryDuration))) {
      ...
      if (ai.cover && (distance(known, ai.cover.hide) < 5 || hasLineOfSight(eye(ai.cover.hide), { ...known, y: 1.3 }))) ai.cover = null;
      ...
      if (ai.role === 'flanker' && dist > 9) {
        if (!ai.flankPlan || distance(ai.flankPlan.contact, known) > 8 || (ai.flankTimer <= 0 && dist > 22)) {
          ...
          ai.flankPlan = { contact: point(known), approach: freeGoal({ x: known.x - dx * 8 - dz * width * e.side, z: known.z - dz * 8 + dx * width * e.side }) };
          ai.flankGoal = freeGoal({ x: e.x - dz * width * e.side + dx * 3, z: e.z + dx * width * e.side + dz * 3 }); ai.flankTimer = 12; ai.flankStage = 0;
        }
      ...
      setGoal(e, dist > 17 ? freeGoal({ x: known.x - dx * 13 - dz * e.side * 2.5, z: known.z - dz * 13 + dx * e.side * 2.5 }) : null); return;
    }
    ai.cover = null;
    if (known) {
      e.mode = 'search';
      ...
      if (ai.searchStage === 0) {
        ...
        const goal = freeGoal({ x: ai.searchOrigin.x + offset, z: ai.searchOrigin.z }); setGoal(e, goal);
        ...
        if (!ai.goal || distance(e, ai.goal) < 1) {
          ...
          setGoal(e, freeGoal({ x: ai.searchOrigin.x + Math.cos(angle) * radius, z: ai.searchOrigin.z + Math.sin(angle) * radius })); ai.searchSector++;
        }
      }
      return;
    }
    ...
    if (!e.patrol || distance(e, e.patrol) < 1.6 || !ai.goal) {
      ...
      e.patrol = freeGoal({ x: e.home.x + Math.cos(angle) * radius, z: e.home.z + Math.sin(angle) * radius });
    }
    setGoal(e, e.patrol);
  }
  ...
  function move(e, enemies, dt) {
    ...
    let next = e.path[0];
    if (next && distance(e, next) < .15) { e.path.shift(); next = e.path[0]; }
    if (!next) {
    ...
    const speed = ai.task === 'patrol' || ai.task === 'return' ? 1.35 : ai.task === 'flank' ? 3.1 : e.mode === 'search' ? 2.8 : 2.35;
    ...
    const nextPoint = { x: e.x + vx, z: e.z + vz };
    if (isWalkable(nextPoint.x, nextPoint.z, .38) && clear(e, nextPoint)) { e.x = nextPoint.x; e.z = nextPoint.z; }
    ...
    for (const e of enemies) {
      ...
      if (ai.visible) {
        ...
        if (target) {
          ...
          if (e.fireTimer <= 0 && distance(e, p) < 30) {
            const targetPoint = { x: p.x, y: (p.y || 0) + (p.crouching ? .95 : 1.3), z: p.z };
            if (hasLineOfSight(eye(e), targetPoint) && hasLineOfSight({ x: e.x, y: 1.43, z: e.z }, targetPoint)) shoot(e, target);
          }
        }
      }
      move(e, enemies, dt);
