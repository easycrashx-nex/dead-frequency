...
import { ROADS, riverCenter } from './terrain.js';
...
const isSupply = item => item.kind === 'ammo' || item.kind === 'medkit';
function drawDeploymentMap(canvas) {
  const ctx=canvas.getContext('2d');if(!ctx)return;
  const scale=canvas.width/layout.size,project=(x,z)=>[canvas.width/2+x*scale,canvas.height/2+z*scale];
  ctx.fillStyle='#252a21';ctx.fillRect(0,0,canvas.width,canvas.height);
  ctx.strokeStyle='#30392b';ctx.lineWidth=1;
  for(let n=-750;n<=750;n+=100){ctx.beginPath();ctx.moveTo(...project(n,-750));ctx.lineTo(...project(n,750));ctx.moveTo(...project(-750,n));ctx.lineTo(...project(750,n));ctx.stroke();}
  ctx.strokeStyle='#111c1b';ctx.lineWidth=72*scale;ctx.beginPath();
  for(let z=-750;z<=750;z+=10){const p=project(riverCenter(z),z);if(z===-750)ctx.moveTo(...p);else ctx.lineTo(...p);}ctx.stroke();
  for(const road of ROADS){ctx.strokeStyle='#6e725a';ctx.lineWidth=3;ctx.beginPath();ctx.moveTo(...project(road.a.x,road.a.z));ctx.lineTo(...project(road.b.x,road.b.z));ctx.stroke();}
  for(const building of layout.interiors){const [x,z]=project(building.x,building.z);ctx.fillStyle='#b4ac7f';ctx.fillRect(x-building.w*scale/2,z-building.d*scale/2,Math.max(2,building.w*scale),Math.max(2,building.d*scale));}
  for(const bridge of layout.bridges||[]){const [x,z]=project(bridge.x,bridge.z);ctx.fillStyle='#ba9a66';ctx.fillRect(x-bridge.w*scale/2,z-bridge.d*scale/2,bridge.w*scale,Math.max(3,bridge.d*scale));}
}
const carriedItemsMarkup = items => items.length ? items.map((item,index) => `<div class="inventory-item rarity-${escapeHTML(item.rarity||'common')}"><span class="inventory-slot">${String(index+1).padStart(2,'0')}</span><div><strong>${escapeHTML(item.name||'Fracht')}</strong><small>${item.kind==='weapon'?'EIGENE WAFFE':item.kind==='equipment'?'AUSRÜSTUNG':item.kind==='attachment'?'WAFFENAUFSATZ':rarityLabel(item.rarity)}</small></div><span class="carried-value">${number(item.value)} <small>CR</small></span><div class="carried-actions">${['weapon','equipment'].includes(item.kind)?`<button class="small-button equip-raid-button" data-equip-raid-item="${escapeHTML(item.id)}" aria-label="${escapeHTML(item.name)} ausrüsten">AUSRÜSTEN ↗</button>`:''}<button class="drop-item-button" data-drop-item="${escapeHTML(item.id)}" aria-label="${escapeHTML(item.name)} abwerfen">ABWERFEN <span>↓</span></button></div></div>`).join('') : `<div class="inventory-empty"><span>▤</span><strong>NOCH KEINE FRACHT.</strong><p>Öffne Kisten in der Zone.<br>Wähle aus, was du mitnehmen möchtest.</p></div>`;
...
const logisticsPanels = `<section id="hub-logistics" class="hub-logistics" hidden>
  <header class="logistics-heading"><div><span class="eyebrow"><span class="orange-dash"></span> DEAD FREQUENCY / BASIS</span><h2 id="logistics-title">DEIN LAGER<span class="orange">.</span></h2><p id="logistics-description">Beute einlagern. Vorräte behalten. Den nächsten Einsatz vorbereiten.</p></div><span class="logistics-stamp">SEKTOR 07<br><b>LOKAL GESICHERT</b></span></header>
...
const controls = `<dl class="controls-grid"><div><dt><kbd data-control-action="look" data-keyboard-label="MAUS">MAUS</kbd></dt><dd>Umsehen</dd></div><div><dt><kbd data-control-action="fire" data-keyboard-label="LMB">LMB</kbd> / <kbd data-control-action="aim" data-keyboard-label="RMB">RMB</kbd></dt><dd>Feuern / Zielen</dd></div>${BINDING_ACTIONS.map(action => `<div><dt><kbd data-binding-code="${action.key}">${escapeHTML(keyLabel(action.default))}</kbd></dt><dd>${escapeHTML(action.label)}</dd></div>`).join('')}<div><dt><kbd data-control-action="pause" data-keyboard-label="ESC">ESC</kbd></dt><dd>Menü / Pause</dd></div><div><dt><kbd>F11</kbd></dt><dd>Vollbild</dd></div></dl>`;

export function createUI(root, actions) {
  root.classList.add('game-interface', 'terminal-interface', 'operator-interface');
  root.innerHTML = `
    <div class="damage-vignette" id="damage-flash" aria-hidden="true"></div>
    <section id="hub-screen" class="screen hub-screen" data-tab="deploy">
      <header class="menu-header"><div class="brand-mark"><span class="signal-icon" aria-hidden="true"><i></i><i></i><i></i><i></i></span> DEAD FREQUENCY <span class="dim">EXTRACTION / SEKTOR 07</span></div><div class="header-right"><span class="online-dot"></span><span id="connection-label">LOKALE OPERATION</span><button class="icon-button" data-action="settings" aria-label="Einstellungen">Einstellungen <span>↗</span></button></div></header>
      ${hubNavigation}<div id="hub-deploy" class="hub-main">
        <button class="deployment-skills" data-hub-tab="skills"><span><small class="micro dim">DEIN OPERATOR</small><strong id="deployment-skill-level">LEVEL 01</strong></span><span><b id="deployment-skill-points">3 SKILLPUNKTE</b><small>Fähigkeiten ansehen ↗</small></span></button>
        <aside class="deployment-panel">
          <div class="deployment-location"><canvas id="deployment-map" width="600" height="250" aria-label="Kartenausschnitt der Küstensperrzone mit Straßen, Siedlungen und Fluss"></canvas><div class="deployment-location-caption"><span class="micro">EINSATZGEBIET</span><h1>Küstensperrzone</h1><span>1,5 × 1,5 km <i>·</i> ${Math.round(RAID_SECONDS/60)} Minuten</span></div></div>
          <div class="deployment-manifest">
            <div class="bank-row"><span id="hub-credits">0</span> CR</div>
            <div class="kit-choices"><button id="deployment-mode-preset" class="kit-choice" data-action="arsenal-kits"><span class="choice-top"><strong>Einsatzkit</strong><span>↗</span></span><span id="deployment-preset-name" class="choice-description">Notfall · Scout</span></button><button id="deployment-mode-custom" class="kit-choice" data-action="arsenal-loadout"><span class="choice-top"><strong>Eigenes Loadout</strong><span>↗</span></span><span class="choice-description">Ausrüstung anpassen</span></button></div>
            <button class="deployment-weapon" data-hub-tab="arsenal" aria-label="Waffe im Arsenal ändern"><span id="deployment-weapon-art"></span><span><small class="micro dim">PRIMÄRWAFFE</small><strong id="deployment-weapon-name">VX-9</strong><small id="deployment-weapon-cost">TEIL DES EINSATZKITS</small></span><b>↗</b></button>
            <div id="deployment-equipped-gear" class="deployment-equipped-gear"></div>
            <div class="difficulty-row"><span class="micro">BEDROHUNG</span><div class="segmented"><button id="difficulty-normal" data-difficulty="normal" class="selected">Normal</button><button id="difficulty-hard" data-difficulty="hard">Hoch</button></div></div>
            <p id="difficulty-description" class="tiny dim difficulty-description">Standardpatrouillen. Ein sauberer Einstieg.</p>
            <div class="deployment-cost"><span>Kosten für diesen Einsatz</span><strong id="deployment-total-cost">0 CR</strong></div>
            <div class="start-row"><button id="start-raid" data-action="start" class="primary-button"><span id="start-label">RAID STARTEN</span><span class="button-arrow">↗</span></button></div>
            <p class="loss-warning" id="deployment-note">Was du nicht extrahierst, bleibt in der Zone.</p>
          </div>
        </aside>
      </div><section id="hub-arsenal" class="hub-feature" hidden></section><section id="hub-skills" class="hub-feature" hidden></section>${logisticsPanels}
      <footer class="menu-footer"><div class="menu-controller-help"><span><kbd data-controller-guide="move">LS</kbd> Wählen</span><span><kbd data-controller-guide="confirm">A</kbd> Bestätigen</span><span><kbd data-controller-guide="back">B</kbd> Zurück</span><span><kbd data-controller-guide="heal">LB</kbd> / <kbd data-controller-guide="inventory">RB</kbd> Bereiche</span></div><div class="footer-hint"><span class="micro"><span data-binding-code="forward">W</span> <span data-binding-code="left">A</span> <span data-binding-code="backward">S</span> <span data-binding-code="right">D</span></span> BEWEGEN <span class="footer-separator">/</span> <span class="micro">MAUS</span> ZIELEN <span class="footer-separator">/</span> <span class="micro" data-binding-code="interact">E</span> INTERAGIEREN</div><button data-action="help" class="text-button">FELDHANDBUCH & STEUERUNG <span>↗</span></button><span class="tiny dim build-label">SINGLEPLAYER · OFFLINE</span></footer>
    </section>
    <section id="raid-hud" class="raid-hud" hidden>
      ...
      <div id="downed-hud" class="downed-hud" role="status" hidden><span class="micro">SCHWER VERWUNDET</span><strong>Hilfe benötigt <b id="bleedout-seconds">60 S</b></strong><p>Dein Partner kann dich mit einem Medkit wiederbeleben.</p></div><div id="revive-hud" class="revive-hud" role="status" hidden><div><strong id="revive-label">WIEDERBELEBUNG</strong><span id="revive-seconds">6,0 S</span></div><div class="revive-track"><i id="revive-fill"></i></div><small id="revive-hint">Interaktion halten · 1 Medkit</small></div><div id="toast-stack" class="toast-stack" aria-live="polite"></div>
      <div class="field-panel" id="map-panel" data-panel="map" hidden><div class="field-panel-header"><div><span class="micro orange">TAKTISCHE ÜBERSICHT</span><h2>SEKTOR 07</h2></div><button class="field-close" data-action="close-field"><kbd data-binding-code="map">M</kbd> / <kbd data-control-action="back" data-keyboard-label="ESC">ESC</kbd> <span>×</span></button></div><div class="map-container"><canvas id="tactical-map" width="720" height="720" aria-label="Taktische Karte mit deiner Position, Funkrelais, Extraktionspunkten sowie begehbaren Gebäuden und ihren Eingängen"></canvas><span class="map-north">N ↑</span></div><div class="map-legend"><span><i class="legend-player"></i>DU</span><span id="map-team-legend" hidden><i class="legend-teammate"></i>TEAM</span><span><i class="legend-relay"></i>FUNKRELAIS</span><span><i class="legend-exfil"></i>EXTRAKTION</span><span><i class="legend-interior"></i>BEGEHBARES GEBÄUDE</span><span><i class="legend-entrance"></i>EINGANG</span></div><p class="field-footnote">Die Zeit läuft weiter. Suche Deckung, bevor du die Karte öffnest.</p></div>
      <section id="container-panel" class="field-panel container-panel" data-panel="container" role="dialog" aria-modal="true" aria-labelledby="container-title" hidden>
        <header class="field-panel-header"><div><span id="container-type" class="micro orange">VERSORGUNG / FUNDSTELLE</span><h2 id="container-title">KISTE DURCHSUCHEN</h2></div><button id="close-container" class="field-close" data-action="close-container" aria-label="Kiste schließen"><kbd data-control-action="back" data-keyboard-label="ESC">ESC</kbd> <span>×</span></button></header>
      ...
      <div class="field-panel inventory-panel" id="inventory-panel" data-panel="inventory" hidden><div class="field-panel-header"><div><span class="micro orange">MITGEFÜHRTE AUSRÜSTUNG</span><h2>RUCKSACK</h2></div><button id="close-inventory" class="field-close" data-action="close-field"><kbd data-binding-code="inventory">TAB</kbd> / <kbd data-control-action="back" data-keyboard-label="ESC">ESC</kbd> <span>×</span></button></div><div class="inventory-summary"><span id="inventory-value">0 CR</span><span id="inventory-capacity" class="micro dim">0 / 6 PLÄTZE</span></div><div class="raid-gear-heading"><span class="micro orange">GETRAGENE AUSRÜSTUNG</span><span class="tiny dim">FESTE KITTEILE BLEIBEN GESPERRT</span></div><div id="raid-equipment-list" class="raid-equipment-list"></div><p class="raid-equipment-hint">Weniger Rucksackplätze? Wirf zuerst überschüssige Beute ab. Schutzplatten benötigen einen Plattenträger.</p><div id="inventory-list" class="inventory-list"></div><div class="inventory-supplies"><span>RESERVEMUNITION <b id="inventory-reserve">90</b></span><span>MEDKITS <b id="inventory-medkits">1</b></span></div><p class="field-footnote">Der Raid läuft weiter. Abgeworfene Ware bleibt hier in der Zone.</p></div>
    </section>
    <section id="pause-screen" class="screen pause-screen" hidden><div class="pause-content"><span class="eyebrow"><span class="orange-dash"></span> VERBINDUNG GEHALTEN</span><h2 id="pause-title">EINSATZ PAUSIERT<span class="orange">.</span></h2><p class="dim" id="pause-description">Durchatmen. Die Zone wartet.</p><button id="resume-raid" class="primary-button" data-action="resume"><span>FORTSETZEN</span><span>↗</span></button><div class="pause-secondary"><button class="secondary-button" data-action="settings">EINSTELLUNGEN</button><button class="secondary-button" data-action="help">STEUERUNG</button></div><button id="abandon-raid" class="text-button abandon-button" data-action="abandon">EINSATZ ABBRECHEN <span>↗</span></button><p id="abandon-note" class="tiny dim">Mitgeführte Beute geht beim Abbruch verloren.</p></div><div id="pause-coordinate" class="pause-coordinate micro dim">DEAD FREQUENCY / SEKTOR 07 / OFFLINE</div></section>
    <section id="result-screen" class="screen result-screen" hidden><div class="result-content"><span class="eyebrow" id="result-eyebrow"><span class="orange-dash"></span> OPERATION ABGESCHLOSSEN</span><h2 id="result-title">ERFOLGREICH EXTRAHIERT<span class="orange">.</span></h2><p id="result-description" class="result-description">Die Fracht ist sicher.</p><div class="result-stats"><div><span class="micro dim">WARENRICHTWERT</span><strong id="result-loot">0 <small>CR</small></strong></div><div><span class="micro dim">BONUS</span><strong id="result-bonus">0 <small>CR</small></strong></div><div><span class="micro dim">ABSCHÜSSE</span><strong id="result-kills">0</strong></div></div><div class="result-xp"><span class="micro dim">OPERATOR-FORTSCHRITT</span><strong id="result-xp">+0 XP</strong></div><div class="result-total"><span class="micro">BONUS DIREKT GUTGESCHRIEBEN</span><strong id="result-total">0 CR</strong></div><button id="result-hub" class="primary-button" data-action="hub"><span id="result-hub-label">ZURÜCK ZUR BASIS</span><span>↗</span></button><p id="result-storage-note" class="tiny dim">Extrahierte Gegenstände warten unter Lager → Anlieferung.</p></div></section>
    <div id="utility-overlay" class="utility-overlay" hidden><section class="utility-dialog" role="dialog" aria-modal="true" aria-labelledby="utility-title"><div class="utility-heading"><div><span class="micro orange">DEAD FREQUENCY / FELDHANDBUCH</span><h2 id="utility-title">STEUERUNG</h2></div><button class="close-button" data-action="close-utility" aria-label="Schließen">×</button></div><div id="help-content"><p class="help-intro">30 Minuten in der Sperrzone. 32 Waffen, 36 Aufsätze und 24 Ausrüstungsteile. Wähle eines von sechs festen Kits oder stelle im Arsenal dein eigenes Loadout zusammen. Extrahiere deine Funde, lagere sie zu Hause ein und verkaufe sie auf dem lokalen Markt.</p><p id="help-interiors" class="help-interiors"><b>${layout.interiors.length} BEGEHBARE GEBÄUDE · ${number(layout.size)} × ${number(layout.size)} M.</b> Häuser in den Dörfern und Gebäude im Hafengebiet durchsuchen. Helle Umrisse und Türpunkte markieren Zugänge; Straßen und Brücken helfen bei der Orientierung.</p><p class="help-interiors"><b>KOOP / WIEDERBELEBEN.</b> Ein verwundeter Partner hat 60 Sekunden Zeit. Mit einem Medkit in Reichweite <span data-binding-code="interact">E</span> sechs Sekunden halten. Schaden, Bewegung aus der Reichweite oder Loslassen unterbrechen die Hilfe.</p>${controls}<div class="help-rules"><p><b>01 / SICHERN</b> Werkzeug, Elektronik, Medizin, Munition, Proviant, Industrie und Sicherheit: Mit <span data-binding-code="interact">E</span> eine Kiste öffnen und kurz durchsuchen. Gegenstände im Kistenfenster nehmen; rechts kannst du Rucksackware abwerfen und Platz schaffen.</p><p><b>02 / SENDEN</b> Das optionale Funkrelais auf der Karte aktivieren und den Bonus sichern.</p><p><b>03 / VERSCHWINDEN</b> Einen markierten Extraktionspunkt erreichen. <span data-binding-code="interact">E</span> drücken und <span id="help-extraction-duration">8</span> Sekunden in der Zone bleiben.</p></div></div><div id="settings-content" hidden></div><button data-action="close-utility" class="secondary-button utility-done">ZURÜCK</button></section></div>
  ...
  function showUtility(kind) {
    ...
    nodes['utility-overlay'].querySelector('.utility-heading .micro').textContent = kind === 'settings' ? 'DEAD FREQUENCY / EINSATZPROFIL' : 'DEAD FREQUENCY / FELDHANDBUCH';
  ...
  const onClick = async event => {
    ...
    if (button.dataset.equipRaidItem) { actions.equipRaidItem?.(button.dataset.equipRaidItem); previousInventory=''; return; }
    if (button.dataset.dropEquipment) { actions.dropEquipment?.(button.dataset.dropEquipment); previousInventory=''; return; }
    ...
    switch (button.dataset.action) {
      case 'start': {
        if(state?.profile?.intake?.length){selectHubTab('storage');return;}
        if(loadoutLocked()){notice('Dein Team startet den Einsatz über die Koop-Lobby.');return;}
  ...
  function drawMap() {
    ...
    ctx.save(); ctx.beginPath(); ctx.rect(margin, margin, size-margin*2, size-margin*2); ctx.clip();
    ctx.strokeStyle = '#29465a'; ctx.lineWidth = 72 * scale; ctx.beginPath();
    for (let z = -layout.size/2; z <= layout.size/2; z += 12) { const [x,y] = point(riverCenter(z),z); if (z === -layout.size/2) ctx.moveTo(x,y); else ctx.lineTo(x,y); }
    ctx.stroke(); ctx.strokeStyle = '#69737a'; ctx.lineWidth = Math.max(1.5, 9*scale);
    for (const road of ROADS) { const a=point(road.a.x,road.a.z), b=point(road.b.x,road.b.z);ctx.beginPath();ctx.moveTo(...a);ctx.lineTo(...b);ctx.stroke(); }
    for (const bridge of layout.bridges || []) { const [x,y]=point(bridge.x-bridge.w/2,bridge.z-bridge.d/2);ctx.fillStyle='#b4a17b';ctx.fillRect(x,y,bridge.w*scale,Math.max(3,bridge.d*scale)); }
    ctx.restore();
    ...
    for (const interior of layout.interiors || []) {
      const [x, y] = point(interior.x-interior.w/2, interior.z-interior.d/2);
      ctx.fillStyle = '#777350'; ctx.strokeStyle = '#e3d4a2'; ctx.lineWidth = 2.5;
      ctx.fillRect(x, y, interior.w * scale, interior.d * scale);
      ctx.strokeRect(x, y, interior.w * scale, interior.d * scale);
    }
    ctx.textAlign = 'center'; ctx.textBaseline = 'middle';
    ctx.font = '600 13px "Segoe UI", sans-serif';
    const mapLabels=[];
    for (const poi of layout.pois || []) {
      let [x, y] = point(poi.x, poi.z);
      const width = ctx.measureText(poi.name.toUpperCase()).width + 12;
      for (const interior of layout.interiors || []) {
        const [left, top] = point(interior.x-interior.w/2, interior.z-interior.d/2);
        if (x+width/2 > left && x-width/2 < left+interior.w*scale && y+10 > top && y-10 < top+interior.d*scale) y = top-19;
      }
      const rect={left:x-width/2,right:x+width/2,top:y-10,bottom:y+10};
      if(mapLabels.some(label=>rect.left<label.right+4&&rect.right>label.left-4&&rect.top<label.bottom+3&&rect.bottom>label.top-3))continue;
      mapLabels.push(rect);
      ctx.fillStyle = '#0b1518cc';
      ctx.fillRect(x-width/2, y-10, width, 20); ctx.fillStyle = '#e6e7d9'; ctx.fillText(poi.name.toUpperCase(), x, y);
    }
    // Door markers stay legible at the full world extent.
    for (const interior of layout.interiors || []) {
      for (const door of interior.doors || []) {
        const [x, y] = point(door.x, door.z);
        ctx.fillStyle = '#fff0c2'; ctx.strokeStyle = '#142019'; ctx.lineWidth = 2.5;
        ctx.beginPath(); ctx.arc(x, y, Math.max(2.5, Math.min(6.5, 2000 / layout.size)), 0, Math.PI*2); ctx.fill(); ctx.stroke();
      }
    }
  ...
  function renderContainer() {
    ...
    if (changed) {
      ...
      searchDuration = container.searchSeconds || state.player?.searchDuration || CONTAINER_SEARCH_SECONDS * getSkillEffects(state.profile).searchMultiplier;
    ...
    const available = items.filter(item => isSupply(item) || !full);
    ...
    setText('container-title', (container.name || type?.name || 'VERSORGUNGSKISTE').toUpperCase());
    setText('container-type', container.kind === 'corpse' ? 'GEFALLENER / AUSRÜSTUNG BERGEN' : type?.description.toUpperCase() || 'VERSORGUNG / FUNDSTELLE');
    nodes['container-panel'].dataset.containerKind = container.kind || 'crate';
  ...
  function renderInventory() {
    ...
    const worn={weapon:player.weaponInstance,...player.equipment};
    const equipmentMarkup=['weapon',...GEAR_SLOTS].map(slot=>{const item=worn[slot],label=({weapon:'WAFFE',backpack:'RUCKSACK',carrier:'TRÄGER',plate:'PLATTE',helmet:'HELM'})[slot];return `<article class="raid-equipped-slot" data-worn-slot="${slot}"><span>${label}</span><strong>${escapeHTML(item?.name||'OHNE')}</strong><small>${item?.issued?'FESTES KITTEIL':item?.condition!==undefined?`${Math.round(item.condition*100)} % ZUSTAND`:item?'EIGENER GEGENSTAND':'FREIER SLOT'}</small><button class="small-button" data-drop-equipment="${slot}"${!item||item.issued?' disabled':''}>ABWERFEN ↓</button></article>`;}).join('');
    if(nodes['raid-equipment-list'].innerHTML!==equipmentMarkup)nodes['raid-equipment-list'].innerHTML=equipmentMarkup;
  ...
  function update(nextState, info = {}) {
    ...
    setText('deployment-preset-name',loadout.selection.mode==='preset'?loadout.name:'Fertige, unveränderbare Einsatzkits');
    setText('deployment-weapon-name',equipped?.name||'WAFFE AUSWÄHLEN');setText('deployment-weapon-cost',loadout.selection.mode==='custom'?'EIGENER BESTAND · MUNITION WIRD AUFGEFÜLLT':'TEIL DES FESTEN EINSATZKITS');
    const weaponArtId=equipped?.id||'';if(nodes['deployment-weapon-art'].dataset.weapon!==weaponArtId){nodes['deployment-weapon-art'].dataset.weapon=weaponArtId;nodes['deployment-weapon-art'].innerHTML=equipped?weaponSilhouette(equipped.id):'';}
    const gearText=`${number(loadout.gearStats.maxArmor+effects.armorBonus)} PANZERUNG · ${number(loadout.gearStats.capacity+effects.capacityBonus)} PLÄTZE · ${number((loadout.medkits||0)+effects.extraMedkits)} MEDKITS`;
    setText('deployment-equipped-gear',gearText);setText('deployment-total-cost',`${number(loadout.cost)} CR`);nodes['deployment-total-cost'].classList.toggle('orange',loadout.cost>(profile.credits||0));
    ...
    if (phase === 'raid') {
      ...
      setText('hud-weapon', typeof p.weapon === 'string' ? p.weapon.toUpperCase() : 'UNBEWAFFNET');
      setText('hud-firemode', !p.weapon ? 'WAFFE AUSRÜSTEN' : p.reload > 0 ? 'NACHLADEN …' : p.ammo === 0 ? 'MAGAZIN LEER' : getWeapon(p.weapon)?.automatic ? 'AUTO' : p.weapon === 'SG-8' ? 'PUMPAKTION' : p.weapon === 'SR-90' ? 'REPETIERER' : 'EINZELFEUER');
      const controllerInput = info.inputDevice === 'controller';
      const sprintKey = controllerInput ? controllerUI.buttonLabel('sprint') : keyLabel(settings.bindings.sprint).toUpperCase();
      const sprintMode = controllerInput ? settings.controllerSprintMode : settings.sprintMode;
      setText('hud-player-action', p.heal > 0 ? 'BEHANDLUNG …' : p.sprinting ? 'SPRINT' : p.sprintExhausted ? sprintMode === 'toggle' ? `ERHOLEN · ${sprintKey} ERNEUT DRÜCKEN` : `${sprintKey} LOSLASSEN` : '');
      ...
      show('crosshair', settings.crosshair && !p.downed && !!info.locked && !panel && !containerOpen && !(p.reload > 0) && !(p.heal > 0));
      const prompt = state.prompt;
      show('interaction', settings.prompts && !p.downed && !!prompt && !panel && !containerOpen && !(raid.extractionProgress > 0));
      if (prompt) { setText('interaction-label', prompt.text); setText('interaction-detail', prompt.kind === 'revive' ? `${info.inputDevice === 'controller' ? controllerUI.buttonLabel('interact') : keyLabel(settings.bindings.interact)} HALTEN · 1 MEDKIT` : prompt.kind === 'extract' ? 'EXTRAKTION ANFORDERN' : prompt.kind === 'relay' ? 'OPTIONALES EINSATZZIEL' : prompt.kind === 'container' ? 'KISTE ÖFFNEN / DURCHSUCHEN' : 'AUFNEHMEN'); }
      show('downed-hud', !!p.downed);
      setText('bleedout-seconds', `${Math.ceil(Math.max(0, p.bleedoutRemaining || 0))} S`);
      const reviveTarget = p.revivingTargetId ? state.teammates?.find(member => member.id === p.revivingTargetId) : p.downed ? p : null;
      const reviveProgress = Math.max(0, reviveTarget?.reviveProgress || p.reviveProgress || 0), reviveDuration = reviveTarget?.reviveDuration || p.reviveDuration || 6;
      show('revive-hud', reviveProgress > 0);
      setText('revive-label', p.downed ? 'DU WIRST WIEDERBELEBT' : 'PARTNER WIEDERBELEBEN');
      setText('revive-seconds', `${Math.max(0, reviveDuration - reviveProgress).toFixed(1).replace('.', ',')} S`);
      setText('revive-hint', p.downed ? 'Dein Partner hilft dir.' : `${info.inputDevice === 'controller' ? controllerUI.buttonLabel('interact') : keyLabel(settings.bindings.interact)} HALTEN · 1 MEDKIT`);
      nodes['revive-fill'].style.width = `${clamp(reviveProgress / reviveDuration * 100, 0, 100)}%`;
    ...
    if ((phase === 'dead' || phase === 'extracted') && state.result) {
      ...
      nodes['result-title'].innerHTML = result.success ? 'ERFOLGREICH EXTRAHIERT<span class="orange">.</span>' : 'SIGNAL VERLOREN<span class="orange">.</span>';
      setText('result-description', result.success ? `${result.itemCount ?? state.raid?.loot?.length ?? 0} Beutestücke und ${result.equipmentCount||0} eigene Ausrüstungsteile gesichert. Lagere deine Funde zu Hause ein.` : result.reason || 'Einsatz beendet. Deine mitgeführte Beute bleibt in der Zone.');
