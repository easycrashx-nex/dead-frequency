...
export function createLoadoutUI(container, actions, {weaponSilhouette}) {
  ...
  const setHTML=(node,html)=>{
    ...
    const active=document.activeElement,identity=active&&node.contains(active)?active.id||active.getAttribute('data-owned-item')||active.getAttribute('data-shop-item')||active.getAttribute('data-preset-kit'):null;
    const oldScroll=node.scrollTop;node.innerHTML=html;node.scrollTop=oldScroll;
    if(identity){const replacement=[...node.querySelectorAll('button,input,select')].find(item=>item.id===identity||item.dataset.ownedItem===identity||item.dataset.shopItem===identity||item.dataset.presetKit===identity);replacement?.focus({preventScroll:true});}
  };
  ...
  container.innerHTML=`<header class="feature-heading"><div><span class="eyebrow"><span class="orange-dash"></span> DEAD FREQUENCY / AUSRÜSTUNG</span><h2>Arsenal</h2><p>Kit auswählen · Bestand ausrüsten · Waffen anpassen</p></div><div class="feature-status"><span class="micro dim">AKTIVES LOADOUT</span><strong id="armory-equipped">SCOUT</strong><small id="armory-cost"></small></div></header><p id="armory-lock-note" class="loadout-lock-note" hidden>Deine Ausrüstung wurde beim Koop-Beitritt festgelegt. Verlasse die Lobby, um sie zu ändern.</p>
    ...
    <section id="armory-kits" class="armory-section"><div class="armory-section-intro"><p>Waffe und Schutz sind fest. Bezahlung pro Einsatz beim Start.</p><span>6 FESTE KITS</span></div><div class="preset-workspace"><div id="preset-kit-list" class="preset-kit-list"></div><aside id="preset-kit-inspector" class="preset-kit-inspector armory-card" aria-label="Gewähltes Einsatzkit"></aside></div><p class="armory-footnote">Leihmaterial bleibt beim Kit. Eigene Funde kannst du bergen und einlagern.</p></section>
    <section id="armory-loadout" class="armory-section" hidden><div class="custom-workspace"><section class="custom-build armory-card"><header><div><span class="micro orange">AUS DEINEM LAGER</span><h3>Eigene Ausrüstung</h3></div><button id="use-custom-loadout" data-loadout-mode="custom" class="small-button accent-button">VERWENDEN ↗</button></header><p class="armory-copy">Aus deinem Lager. Bei Tod geht die mitgenommene Ausrüstung verloren.</p><div id="custom-equipment-slots">${['weapon',...slots].map(slot=>`<div class="custom-equipment-row"><span class="custom-slot-icon">${slot==='weapon'?weaponSilhouette(WEAPONS[0].id):gearSilhouette(slot)}</span><label for="loadout-${slot}"><span>${esc(gearNames[slot]||slot)}</span><select id="loadout-${slot}" data-equip-slot="${slot}" aria-label="${esc(gearNames[slot]||slot)} für eigenes Loadout"></select></label>${slot==='weapon'?'<button id="edit-equipped-weapon" class="small-button" title="Eigene Waffe bearbeiten">BEARBEITEN ↗</button>':''}</div>`).join('')}</div><div class="custom-supplies"><label for="custom-medkits">MEDKITS <small>25 CR / STÜCK</small></label><select id="custom-medkits">${[0,1,2,3,4].map(n=>option(n,`${n} Medkits · ${n*25} CR`)).join('')}</select></div><div id="custom-stat-summary" class="custom-stat-summary"></div><p id="custom-loadout-error" class="armory-warning"></p><div class="custom-cost-line"><span>MUNITION + MEDKITS PRO RAID</span><strong id="custom-supply-cost">0 CR</strong></div></section><section class="owned-equipment armory-card"><header><div><span class="micro dim">SICHER IN DER BASIS</span><h3>Bestand <small id="owned-count">0</small></h3></div><button data-armory-tab="shop" class="small-button">SHOP ↗</button></header><div class="owned-filter"><label for="owned-kind" class="sr-only">Bestand filtern</label><select id="owned-kind"><option value="all">Alle Ausrüstung</option value="weapon">Waffen</option><option value="attachment">Aufsätze</option><option value="equipment">Schutz & Rucksäcke</option></select></div><div id="owned-equipment-list" class="owned-equipment-list"></div><p class="armory-footnote">ANBIETEN öffnet den Markt. Du legst Preis und Laufzeit selbst fest.</p></section></div></section>
    <section id="armory-editor" class="armory-section" hidden><div class="editor-toolbar"><label for="editor-weapon"><span class="micro dim">DEINE WAFFENINSTANZ</span><select id="editor-weapon"></select></label><button id="equip-editor-weapon" class="small-button">IM LOADOUT EINPLANEN ↗</button><button data-armory-tab="shop" class="small-button">WAFFE KAUFEN ↗</button></div><div id="editor-empty" class="armory-empty"><span>◇</span><strong>Keine eigene Waffe</strong><p>Kaufe eine eigene Waffe im Shop oder extrahiere einen Fund.<br>Fertige Kits werden hier nicht verändert.</p><button data-armory-tab="shop" class="secondary-button">ZUM WAFFENSHOP ↗</button></div><div id="editor-workspace" class="editor-workspace"><section class="weapon-inspection armory-card"><div class="editor-model-header"><div><span id="editor-category" class="micro orange"></span><h3 id="editor-weapon-name"></h3></div><span id="editor-build-state" class="editor-build-state">MONTIERT</span></div><div id="weapon-preview" class="weapon-preview" aria-label="Drehbare 3D-Vorschau der gewählten Waffe"><div class="preview-fallback" id="weapon-preview-fallback"></div></div><div class="weapon-preview-controls"><span id="editor-attachment-count" class="micro dim">0 / 6 AUFSÄTZE</span><div><button id="preview-left" class="small-button" aria-label="Waffe nach links drehen">↶ DREHEN</button><button id="preview-reset" class="small-button" aria-label="Waffenansicht zurücksetzen">RESET</button><button id="preview-right" class="small-button" aria-label="Waffe nach rechts drehen">DREHEN ↷</button></div></div><div class="editor-stat-heading"><span>WAFFENWERTE</span><span>MONTIERT → VORSCHAU</span></div><dl id="editor-stats" class="editor-stats"></dl><p class="armory-footnote">Waffenwerte ohne Operator-Skills. Grün verbessert den gewählten Wert; Orange markiert einen Nachteil.</p></section><section class="weapon-modifications armory-card"><header><div><span class="micro orange">ANPASSUNG / 6 SLOTS</span><h3>Aufsätze</h3></div></header><div class="attachment-slot-grid">${attachmentSlots.map(slot=>`<button data-attachment-slot="${esc(slot)}" aria-pressed="${slot===attachmentSlot}"><span>${esc(slotName(slot))}</span><strong data-slot-installed="${esc(slot)}">STANDARD</strong></button>`).join('')}</div><label class="armory-label" for="editor-attachment">PASSENDER AUFSATZ AUS DEM LAGER</label><select id="editor-attachment"></select><div id="attachment-detail" class="attachment-detail"></div><p id="attachment-compatibility" class="armory-copy"></p><div class="attachment-actions"><button id="mount-attachment" class="primary-button">MONTIEREN <span>↗</span></button><button id="unmount-attachment" class="secondary-button">ABMONTIEREN</button></div><button id="shop-compatible" class="text-button">PASSENDE AUFSÄTZE IM SHOP ↗</button><p class="armory-footnote">Ausgewechselte Teile kehren ins Lager zurück.</p></section></div></section>
    <section id="armory-shop" class="armory-section" hidden><div class="shop-filters"><label class="shop-search" for="shop-search"><span>⌕</span><input id="shop-search" type="search" placeholder="Waffe, Aufsatz oder Ausrüstung suchen …" aria-label="Shop durchsuchen" autocomplete="off"></label><select id="shop-kind" aria-label="Shop-Art"><option value="weapon">Waffen</option><option value="attachment">Aufsätze</option><option value="equipment">Schutz & Rucksäcke</option></select><select id="shop-category" aria-label="Shop-Kategorie"></select></div><label class="shop-compatible-toggle" id="shop-compatible-label" hidden><input type="checkbox" id="shop-compatible-only"><span id="shop-compatible-text">Nur kompatible Aufsätze für die gewählte eigene Waffe</span></label><div class="shop-workspace"><div id="shop-catalog-list" class="shop-catalog-list" aria-label="Shopangebote"></div><section id="shop-detail" class="shop-detail armory-card"><span id="shop-item-category" class="micro orange"></span><div id="shop-item-art"></div><h3 id="shop-item-name"></h3><p id="shop-item-description" class="armory-copy"></p><dl id="shop-item-stats" class="shop-item-stats"></dl><p id="shop-item-compatibility" class="armory-copy"></p><div class="shop-purchase-summary"><span id="shop-owned-count"></span><strong id="shop-item-price"></strong></div><button id="purchase-equipment" class="primary-button">KAUFEN & EINLAGERN <span>↗</span></button><p id="shop-purchase-note" class="armory-footnote">Eigener Gegenstand · danach unter Ausrüstung anlegen.</p></section></div></section>`;

  const comparison=document.createElement('aside');comparison.className='editor-comparison armory-card';comparison.setAttribute('aria-label','Waffenwerte vergleichen');
  comparison.append(container.querySelector('.editor-stat-heading'),el('editor-stats'));
  const comparisonNote=document.createElement('p');comparisonNote.className='armory-footnote';comparisonNote.textContent='Ohne Operator-Skills · Grün: Vorteil · Amber: Nachteil';comparison.append(comparisonNote);
  container.querySelector('.weapon-inspection > .armory-footnote').remove();el('editor-workspace').append(comparison);
  ...
  function renderKits() {
    const kit=PRESET_KITS.find(value=>value.id===profile.loadout?.presetId)||PRESET_KITS[0];
    setHTML(el('preset-kit-list'),PRESET_KITS.map((item,index)=>{
      const chosen=profile.loadout?.mode==='preset'&&profile.loadout.presetId===item.id;
      return `<article class="preset-kit-card${chosen?' selected':''}" data-kit-card="${esc(item.id)}"><span class="preset-index">${String(index+1).padStart(2,'0')}</span>${weaponSilhouette(item.weapon)}<div class="preset-title"><h3>${esc(item.name)}</h3><span>${esc(getWeapon(item.weapon)?.name||item.weapon)}</span></div><strong class="preset-row-cost">${item.cost?`${num(item.cost)} CR`:'KOSTENLOS'}</strong><button data-preset-kit="${esc(item.id)}" class="weapon-select" aria-label="${esc(item.name)} wählen" aria-pressed="${chosen}" ${context.locked?'disabled':''}>${chosen?'GEWÄHLT':'WÄHLEN'}</button></article>`;
    }).join(''));
    const resolved=resolveLoadout(profile,{mode:'preset',presetId:kit.id}),weapon=resolved.weapon;
    setHTML(el('preset-kit-inspector'),`<header><span class="micro orange">FESTER AUFBAU</span><span class="micro dim">${kit.cost?'EINSATZKIT':'NOTFALLKIT'}</span></header><h3>${esc(kit.name)}</h3><p class="armory-copy">${esc(kit.description)}</p><div class="preset-inspector-weapon">${weaponSilhouette(weapon?.id||kit.weapon)}<strong>${esc(weapon?.name||kit.weapon)}</strong></div><dl class="shop-item-stats">${statMarkup(weapon)}</dl><p class="preset-parts">${Object.values(weapon?.attachments||{}).map(id=>getAttachment(id)?.name).filter(Boolean).map(esc).join(' · ')||'Standardaufbau ohne zusätzliche Aufsätze'}</p><div class="preset-gear">${slots.map(slot=>`<span><small>${esc(gearNames[slot])}</small>${esc(resolved.equipment?.[slot]?.name||'Ohne')}</span>`).join('')}</div><div class="preset-summary"><span>${num(resolved.gearStats?.maxArmor)} PANZERUNG</span><span>${num(resolved.gearStats?.capacity)} PLÄTZE</span><span>${num(resolved.medkits)} MEDKITS</span></div>`);
  }
  ...
  function renderShop(resetCategories=false) {
    ...
    text('shop-purchase-note',context.locked?'Im Koop festgelegt. Verlasse die Lobby, um Ausrüstung zu kaufen.':Number(profile.credits||0)<price(item)?`Es fehlen ${num(price(item)-Number(profile.credits||0))} CR für diesen Kauf.`:'Eigener Gegenstand · danach unter Ausrüstung anlegen.');
  }
  function render() {
