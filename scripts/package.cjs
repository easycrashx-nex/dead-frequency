...
async function main(){
  ...
  for(const name of ['simulation.js','layout.js','loot-catalog.js','economy.js','coop-session.js'])await fs.copyFile(path.join(project,'src',name),path.join(stage,'src',name));
