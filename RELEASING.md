...
3. Die neue Windows-Ausgabe mit `node scripts/qa-social-native.mjs` gegen den aktualisierten dedizierten Server prüfen. `DF_QA_ACCOUNTS` kann auf eine private Datei mit zwei bereits registrierten QA-Accounts zeigen; standardmäßig verwendet der Test die isolierten Accounts des 1.12-Prüflaufs. Der Test steuert zwei echte Windows-Instanzen durch Freundschaft, Einladung, gemeinsamen Raid und öffentliche Lobbyliste. Zugangsdaten in `accounts.private.json` bleiben ausschließlich außerhalb des Repositorys. Für Änderungen an Konto und Käufen zusätzlich `node scripts/qa-online-native.mjs` verwenden. Der lokale Koop-Regressionslauf ist `node scripts/qa-coop-native.mjs`. Weitere gezielte Prüfungen passend zur Änderung ausführen.
4. `python scripts/release.py --qa-report ../qa-social-native-<VERSION>/result.json` erzeugt das vollständige Windows-ZIP, den Quellcode und `SHA256SUMS.txt`. Bei anderen Prüfabläufen stattdessen den passenden erfolgreichen nativen Bericht angeben.
